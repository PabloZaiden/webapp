...
`getBunCompileTargetFromArgs` accepts at most one `--target=<value>` argument. Supported values are exposed by `BUN_COMPILE_TARGETS` from `@pablozaiden/webapp/build`: `bun-linux-x64`, `bun-linux-arm64`, `bun-darwin-x64`, `bun-darwin-arm64`, and `bun-windows-x64`. Empty, malformed, duplicate, and unsupported values fail before the builder creates output directories or invokes Bun. Omit the option for the default local Bun binary.
